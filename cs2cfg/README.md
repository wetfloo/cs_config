...
| `sound_*`            | Controls master volume state                                                  | `on`          |
| `crosshair_recoil_*` | Controls if the crosshair should follow recoil or not                         | `on`          |
| `hand_dyn_*`         | `on` sets the viewmodel hand per-slot, `off` uses the same hand for all slots | `on`          |
